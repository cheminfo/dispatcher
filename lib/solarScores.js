...
module.exports.allFromGroup = function(group) {
    ...
    var luminosity = getParams(devices, 'luminosity');
    var extLuminosity = getParams(extDevices, 'luminosity');
    ...
    // 4: luminosity
    if(!luminosity.length || !extLuminosity.length) {
        prom.push(Promise.resolve('not available'));
    } else {
        prom.push(doLuminosity(luminosity, extLuminosity));
    }
    ...
    return Promise.all(prom).then(function(results) {
        ...
        finalRes.luminosity = results[4];
        return finalRes;
    });
};

function doLuminosity(luminosity, extLuminosity) {
    var prom = [];
    prom.push(getData(extLuminosity[0]));
    prom.push(getData(luminosity[0]));

    return Promise.all(prom).then(function(data) {
        var score = new solarScores.Luminosity();
        score.setReference(data[0], extLuminosity[0].param);
        score.setInternal(data[1], luminosity[0].param);
