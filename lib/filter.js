...
Filter.prototype.chartFromDatabaseEntries = function(dbEntries) {
    ...
    if(isMean) {
        return chartFromMeanDbEntries(dbEntries);
    }
    else {
        return chartFromDbEntries(dbEntries);
    }
};
...
function chartFromMeanDbEntries(dbEntries) {
    var chart = {
        type: 'chart',
        value: {
            title: 'data',
    ...
    for(var i=0; i<keys.length; i++) {
        ...
        for(var j=0; j<dbEntries.length; j++) {
            ...
            max.push(dbEntries[j][keys[i].replace('_mean', '_max')]);
        }
...
function chartFromDbEntries(dbEntries) {
    var chart = {
        type: 'chart',
        value: {
            title: 'data',
